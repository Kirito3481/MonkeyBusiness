"""SOUND VOLTEX EXCEED GEAR: game.sv6_*. The work is done by SdvxGame (base.py); what belongs to this version only
is set here (the event flags; its skill courses are data, data/6/skill_courses.json). Which profile and which
...
# soundvoltex.dll registers "game" on the service "local2" up to EXCEED GEAR (ess.dll's eventlog too, see eventlog.py).
...
game = SdvxGame(version="6", events=EVENTS)
...
@router.post("/{gameinfo}/game/sv6_save_c")
async def game_sv6_save_c(request: Request):
